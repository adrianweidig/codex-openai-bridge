...
Bei `stream=true` sendet die Bridge sofort Responses-API-SSE-Events und sichtbare Markdown-Fortschrittsdeltas. Die Bridge nutzt dafür `codex exec --json` und zeigt eine aktive Chat-Ansicht mit Session-Start, Bearbeitungsstart, Denken-Status, verständlich beschriebenen Shell-/Tool-Schritten, komprimierten Befehlsausgaben, Abschluss und Token-Nutzung. Lange Codex-JSON-Events werden erst nach dem Parsen gekürzt, damit große Befehlsausgaben nicht den Live-Status zerstören. Versteckte Modellgedanken werden nicht offengelegt; die Bridge zeigt nur sichere Statuszusammenfassungen. Wenn der Client den Chat abbricht, stoppt die Bridge den laufenden Codex-Prozess und behandelt das als normalen Abbruch. Das Heartbeat-Intervall wird über `CODEX_BRIDGE_PROGRESS_INTERVAL` gesteuert.
